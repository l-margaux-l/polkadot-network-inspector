...
async def collect_and_print_metrics(
    node: Node, 
    ...
    if metrics:
        ...
        logger.error(f"Failed to collect metrics for {node.name}")  


async def main():
    """Main entry point with CLI support."""
    logger = setup_logger("polkadot-inspector", log_dir=config.LOG_DIR)
    logger.info("Polkadot Network Inspector started")
    ...
    if args.list:
        ...
        logger.info("Listed available nodes")  
        return
    ...
    try:
        ...
        if args.all_nodes:
            ...
            logger.info(f"Monitoring {len(nodes_to_monitor)} nodes (--all-nodes)")  

        elif args.node:
            node = ConfigLoader.get_node_by_name(args.node)
            if node:
                nodes_to_monitor = [node]
                logger.info(f"Monitoring node: {args.node}")  
            else:
                print(f"Error: Node '{args.node}' not found")
                logger.error(f"Node '{args.node}' not found") 
                ConfigLoader.list_available_nodes()
                return
            ...
            if default_node:
                ...
                logger.info("Using default node: polkadot_main")  
            else:
        ...
        for node in nodes_to_monitor:
            await collect_and_print_metrics(node, collector, logger)  
        ...
        logger.info("Polkadot Network Inspector stopped")  
